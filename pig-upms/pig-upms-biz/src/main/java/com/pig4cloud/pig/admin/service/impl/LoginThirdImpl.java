...
@RefreshScope
@AllArgsConstructor
public class LoginThirdImpl implements LoginHandler {
	...
	public LoginDto login(LoginVo vo) {
		SysUser sysUser = noTenantSysUserMapper.selectOne(new LambdaQueryWrapper<SysUser>()
				.eq(SysUser::getAccessKey, vo.getAccessKey())
		...
		if (!vo.getSecretKey().equals(sysUser.getSecretKey())) {
			log.error("secretKey校验错误");
			throw new RuntimeException("参数校验错误");
		}
		return getToken(sysUser.getUsername(), sysUser.getSecretKey());
	}

	/**
	 * 获取token
	 */
	private LoginDto getToken(String username, String secretKey) {
		LinkedMultiValueMap<String, String> body = new LinkedMultiValueMap<>();
		body.add(EnumAuth.GRANT_TYPE.getDesc(), EnumAuth.THIRD.getDesc());
		body.add(EnumAuth.USERNAME.getDesc(), String.valueOf(username));
		body.add(EnumAuth.SECRET_KEY.getDesc(), String.valueOf(secretKey));
